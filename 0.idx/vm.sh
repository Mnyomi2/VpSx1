#!/bin/bash
# nix-shell -p qemu wget cloud-utils --run "chmod +x vm.sh && ./vm.sh"
# nohup bash -c "curl -sSf https://sshx.io/get | sh -s run" >/dev/null 2>&1 &
# sudo apt  install docker.io 

# nohup bash -c "curl -sSf https://sshx.io/get | sh -s run" >sshx.log 2>&1 &

# tail -n 20 sshx.log

# cd /run/vms
# nix-shell -p pv
# tar -cf - * | pv | gzip > /var/ubuntu24-chromeremote-qume_backupp.tar.gz 
# tar -cf - * | pv | gzip -9 > /var/ubuntu24-chromeremote-qume_backup3.tar.gz
# tar -cf - * | pv | xz -9 - > /var/ubuntu22-xfce4-chromeremote-qume_backup.tar.xz
# tar -cf - * | pv | gzip -9 > /var/ubuntu22-foldershare-chromeremote-qume_backup3.tar.gz
# tar -xzvf ubuntu22-xfce4-chromeremote-qume_backup.tar.gz -C /baynar
# tar -xJvf ubuntu22-xfce4-chromeremote-qume_backup.tar.xz -C /baynar
# tar -cf - * | pv | gzip -9 > /var/vms/ubuntu24-without-chromeremote-qume_backup.tar.gz

# pv ~/vms/ubuntu24-chromeremote-qume_backupp.tar.gz | tar xzf - -C ~/var
# tar -xzvf /run/ubuntu24-chromeremote-qume_backupp.tar.gz -C /var/vms
# cp /var/ubuntu22-xfce4-chromeremote-qume_backup.tar.xz ~/vms/

# cd /var/vms && rm ubuntu24.conf  ubuntu24.img  ubuntu24-seed.iso
# pkill -f qemu-system-x86_64

# share a file via the internet from your Nix/Linux system
